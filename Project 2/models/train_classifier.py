...
def tokenize(text):
    ...
    for token in tokenz:
        data.append(words.lemmatize(token).lower().strip())
    return data
...
def evaluate_model(model, X_test, Y_test, category_names):
    ...
    print(classification_report(Y_test.values, model.predict(X_test), target_names=category_names))


def save_model(model, model_filepath):
    # save model to pickle file
    pOpen = open(model_filepath, "wb")
    pickle.dump(model, pOpen)
